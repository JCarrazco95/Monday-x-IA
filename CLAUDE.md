...
- **nextBestActionAgent.ts** — **determinista** (sin IA). Recorre `logs` y levanta alertas de seguimiento: compromisos sin seguimiento/vencidos, leads calientes/tibios enfriándose, llamadas con banderas rojas. Escribe las de alta prioridad a Monday (columna `requiere_atencion` + comentario) → automatizaciones nativas notifican. Rutas: `GET /api/nba` (preview, no escribe) y `POST /api/nba/run` (escribe). "Cron" interno opcional vía `NBA_CRON_HOURS`. Umbrales `NBA_HORAS_*`. Frontend: `pages/NextBestAction.tsx` (`/seguimiento`). **Fix importante**: la "última actividad" por referencia se calcula SOLO con agentes de negocio (`lead_enrichment`/`form_analysis`/`call_intelligence`); antes incluía orchestrator/writer (que loguean segundos después del análisis) y la condición "la llamada es la última actividad" nunca se cumplía → las reglas de compromisos y llamadas en riesgo jamás disparaban.
...
- `coaching.ts`: `GET /api/coaching[?vendedor=&dias=]` → agregación sobre llamadas analizadas (Sandler/Challenger/Global prom, etapa Sandler más débil, perfiles Challenger, radar de habilidades, banderas rojas/objeciones recurrentes, tendencia mensual). **Identidad del vendedor por llamada** (Fase 3, hecha): cada `CallIntelligenceOutput` guarda `ejecutivo` (user de Aircall vía `detail.agente`, o capturado a mano en las rutas de ingesta con `ejecutivo` en el body; el simulate demo rota 3 vendedores ficticios). Sin filtros = equipo; `?vendedor=` acota todo a ese vendedor; `?dias=` al periodo. La respuesta siempre trae `vendedores` (para el selector) y `ranking` (comparativa por vendedor, calculada sobre el periodo sin filtro de vendedor). Frontend `pages/Coaching.tsx` (`/coaching`, admin): selectores de vendedor/periodo + tabla comparativa clicable. La lista de Call Intelligence muestra columna Vendedor. Cache de análisis: si el análisis viejo no traía `ejecutivo` y la nueva ingesta sí, se completa sin re-analizar.
- `forecast.ts`: `GET /api/forecast` → **pipeline con datos REALES de Monday** cuando hay `MONDAY_API_TOKEN` + `MONDAY_BOARD_ID_OPORTUNIDADES` (board 8311006777 "Oportunidades Maxirent"; lector solo-lectura en `lib/mondayForecast.ts`, paginado, cero mutations). Montos reales (`Valor del acuerdo` con fallback `Valor de la cotizacion`), etapas reales (grupo manda sobre status en cierres), fecha estimada de cierre, ganado mes/año (fecha real de cierre), `porEjecutivo` y objetivos mensuales del board 8311006715 (mejor esfuerzo: hoy los items no son visibles para el token → `objetivos.disponible:false` con motivo). Único supuesto: probabilidad por etapa (`FORECAST_PROB_ETAPAS`, def. Cotización 30 / Req. seguimiento 20 / Negociando 60 / Documentación 85). Live estricto: si Monday falla → 502, sin fallback silencioso. Sin token → modo `estimado` anterior (bitácora, `FORECAST_TICKET_BASE × factor(score)`). La respuesta declara `fuente: monday|estimado` y el frontend lo muestra como badge. IDs de columna con defaults reales, sobreescribibles vía `FORECAST_COL_*`. Boards de referencia: Leads 8311006929 · Oportunidades 8311006777 · Objetivos 8311006715 (dashboard 37572358 NO es legible por API; se leen sus boards fuente). Frontend `pages/Pipeline.tsx` (`/pipeline`).
...
- **Tablero de llamadas de Aircall en Monday (sincronización)**: Aircall registra cada llamada como item en un board propio (`MONDAY_BOARD_ID_CALLS`, def. `18398458590`) con columnas call id (`MONDAY_COL_CALL_ID`=`text_mm07x5tn`), link a la llamada (`MONDAY_COL_CALL_LINK`=`link_mm07s8jf`) y relación al lead (`MONDAY_COL_CALL_LEAD`=`board_relation_mm1whczc`). `monday.getCallsBoardItems()` lee esos items (con `startedAt`, ordenados por más recientes); `syncCallsBoard({max?, sinceISO?})` (en `aircallIngest.ts`) analiza las NUEVAS. La transcripción sale del **link de grabación** (`assets.aircall.io/calls/<idNumérico>/recording`): de ahí se extrae el **ID numérico de Aircall** (la columna Call ID trae el SID de Twilio, inservible para la API) y se transcribe con **Deepgram** (Aircall AI no expone transcript por API en esta cuenta). **Idempotente** (dedupe por `aircall-<id>`/`url-<hash>` en la bitácora → nunca re-analiza). Tope `CALLS_SYNC_MAX` (def. 25); corte por fecha `CALLS_SYNC_SINCE` (solo de esa fecha en adelante). **Automático**: cron interno `CALLS_SYNC_CRON_HOURS` (en `index.ts`, `scheduleCallsSync`) — sin botón. El Writer **NUNCA escribe en Monday** para llamadas (itemId no numérico → se omite toda escritura; el análisis solo va a la bitácora). Rutas: `GET /api/calls/board` (preview), `POST /api/calls/sync-board` (body `{max, since}`).
...
- **`MONDAY_READ_ONLY=true`** — candado global: las lecturas (forecast, sync, columnas) funcionan
  normal pero TODA mutation a Monday se simula en el transporte (`mondayRequest` detecta `mutation`)
  y queda solo en la bitácora. `/api/health` reporta `mondayMode: live-readonly`. Activado en el
  `.env` local porque el token apunta a boards de PRODUCCIÓN de MAXIRent; apagarlo solo cuando se
  mapee `MONDAY_COLUMN_MAP` (p. ej. `requiere_atencion`) y el cliente autorice escrituras.
- **⚠️ BOARDS: producción vs prueba.** `8311006929` "Leads Maxirent" es el board REAL de la
  empresa: **NUNCA se escribe ahí, bajo ninguna circunstancia** (solo lectura). El board de
  escritura/pruebas es `18419649847` "Leads" (workspace de prueba) → `MONDAY_BOARD_ID_LEADS`
  apunta SIEMPRE al de prueba. Oportunidades `8311006777` y Objetivos `8311006715` también son
  de producción, solo lectura (los lee el forecast).
- **Forecast live:** `MONDAY_BOARD_ID_OPORTUNIDADES`, `MONDAY_BOARD_ID_OBJETIVOS`, `FORECAST_COL_*`,
  `FORECAST_PROB_ETAPAS` (ver `.env.example`).
